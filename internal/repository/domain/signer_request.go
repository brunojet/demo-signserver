...
const (
	SignerStatusCreated           SignerStatus = "created"
...
func (s *SignRequest) GetLastError() *SignerError {
	...
	for i := len(*s.History) - 1; i >= 0; i-- {
		h := (*s.History)[i]
		if h.SignerStatus != nil && *h.SignerStatus == SignerStatusSigningFailed {
			return h.Error
		}
	}
	return nil
