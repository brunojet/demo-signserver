...
func TestDynamoDBService_CreateItem(t *testing.T) {
	...
	item := Item{}
	item.SetID(pk_value)
	err := dynamo_service_test.CreateItem(context.Background(), &item)
...
func TestDynamoDBService_GetItem(t *testing.T) {
	...
	assert.NoError(t, err)
	var out Item
	err = dynamo_service_test.GetItem(context.Background(), item.ID, &out)
	assert.NoError(t, err)
...
func TestDynamoDBService_GetItem_Error(t *testing.T) {
	...
	assert.NoError(t, err)
	var out Item
	err = dynamo_service_test.GetItem(context.Background(), item.ID+"1", &out)
	assert.Error(t, err)
}
...
func TestDynamoDBService_UpdateItem(t *testing.T) {
	...
	err = dynamo_service_test.UpdateItem(context.Background(), item.ID, &update)
	assert.NoError(t, err)
...
func TestDynamoDBService_UpdateItem_Error(t *testing.T) {
	...
	// Atualiza campo Name
	update := Item{Name: "updated"}
	err = dynamo_service_test.UpdateItem(context.Background(), item.ID+"1", &update)
	assert.Error(t, err)
}

func TestNewDynamoDBService_Coverage(t *testing.T) {
	tableName := "table_coverage"
	table_service_test.CreateTable(context.Background(), tableName, SORT_KEY)
	defer table_service_test.DeleteTable(context.Background(), tableName)
